...
class ApiClient {
  ...
  auth = {
    ...
    emailExists: (email: string) =>
      this.request<{ exists: boolean }>(`/auth/email-exists?email=${encodeURIComponent(email)}`),

    phoneExists: (phone: string) =>
      this.request<{ exists: boolean }>(`/auth/phone-exists?phone=${encodeURIComponent(phone)}`),
  };
  ...
  // ============ FRAUD LOGS ============
  fraudLogs = {
    getAll: (params?: { aadhaar_record_id?: string; start_date?: string; end_date?: string; resolved?: boolean }) => {
      const query = params ? '?' + new URLSearchParams(params as any).toString() : '';
      return this.request<any[]>(`/fraud-logs${query}`);
    },

    getById: (id: string) => this.request<any>(`/fraud-logs/${id}`),

    create: (data: any) => this.request<any>('/fraud-logs', { method: 'POST', body: data }),

    resolve: (id: string) => this.request<any>(`/fraud-logs/${id}/resolve`, { method: 'PUT' }),

    getUnresolvedCount: () => this.request<{ count: number }>('/fraud-logs/stats/unresolved-count'),
  };
}
